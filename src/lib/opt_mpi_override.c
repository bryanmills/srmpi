...
int MPI_Init(int *argc, char ***argv) {
    ...
    // ignore error
    if (rank == 0) {
        printf("We are running optimized collective library - bnm\n");
        fflush(NULL);
    }
}
#endif
...
int MPI_Bcast( void *buf, int cnt, MPI_Datatype dt, int root,
    ...
    return MPI_Bcast_log(buf, cnt, dt, root, comm);
}

#ifndef SIMPLE_REPLICA_NODES
int MPI_Barrier( MPI_Comm comm ) {

    return MPI_Barrier_log(comm);
}
#endif
                ...
                MPI_Op op, int root, MPI_Comm comm )
{
    ...
    return MPI_Reduce_log(sbuf, rbuf, cnt, dt, op, root, comm);
}
...
int MPI_Gather( void *sbuf, int scnt, MPI_Datatype sdt,
    ...
    return MPI_Gather_binomial(sbuf, scnt, sdt, rbuf, rcnt, rdt, root, comm);
}
...
int MPI_Scatter( void *sbuf, int scnt, MPI_Datatype sdt,
    ...
    return MPI_Scatter_binomial(sbuf, scnt, sdt, rbuf, rcnt, rdt, root, comm);
}
...
{
    return MPI_Reduce_scatter_halving(sbuf, rbuf, rcnts,
                                      dt, op, comm);
}
...
int MPI_Alltoall( void *sbuf, int scnt, MPI_Datatype sdt,
    ...
    return MPI_Alltoall_pairwise(sbuf, scnt, sdt, rbuf, rcnt, rdt, comm);
}
...
MPI_Alltoallv( void *sbuf, int *scnts, int *sdispls, MPI_Datatype sdt,
               ...
               MPI_Comm comm )
{
    return MPI_Alltoallv_pairwise(sbuf, scnts, sdispls, sdt,
